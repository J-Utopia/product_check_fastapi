...
def test_normalizer_strips_style_noise_from_included_excluded() -> None:
    raw = {
        "package_info": {
            "pName": "테스트 상품",
            "date": {"sdate": "2026-06-20", "edate": "2026-06-24", "night": 4, "days": 5},
            "air": {"countryName": "중국"},
        },
        "detail": {
            ...
            "travelNight": 4,
            "travelDays": 5,
            "includedNote": "<style>body{font-family:Pretendard;}</style><p>왕복항공권</p><p>숙박비</p>",
            "unincludedNote": "<style>.x{color:red;}</style><p>가이드/기사 경비</p>",
            "hashtags": ["#내몽고", "#사막숙박"],
            "accumulationExpectedTourMileage": "예상 마일리지 1000점",
        },
    ...
    assert product.included_items == ["왕복항공권", "숙박비"]
    assert product.excluded_items == ["가이드", "기사 경비"]
    assert product.hashtags == ["#내몽고", "#사막숙박"]
    assert product.expected_tour_mileage_text == "예상 마일리지 1000점"
