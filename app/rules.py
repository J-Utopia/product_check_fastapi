...
SHOPPING_PRESENT_PATTERNS = ("쇼핑센터", "쇼핑", "구매", "판매장", "특산품점")
                ...
                if _contains_any(day_text, OPTION_PRESENT_PATTERNS) and product.optional_tour_or_not == "N":
                    issues.append(
                        _issue(
                            ...
                            "ERROR",
                            "선택관광 여부 충돌",
                            f"{day.day_no}일차 일정에는 선택관광 또는 유료 옵션 표현이 있으나 상품상 선택관광 없음으로 보입니다.",
                ...
                if _contains_any(day_text, SHOPPING_PRESENT_PATTERNS) and (product.shopping_count or 0) == 0:
                    issues.append(
                        _issue(
                            ...
                            "ERROR",
                            "쇼핑 여부 충돌",
                            f"{day.day_no}일차 일정에는 쇼핑성 표현이 있으나 상품상 쇼핑 횟수가 0회로 보입니다.",
        ...
        if any("�" in target for target in text_targets if target):
            issues.append(
                _issue(
                    ...
                    "원문 HTML 또는 API 원본 텍스트 인코딩을 다시 확인하세요.",
